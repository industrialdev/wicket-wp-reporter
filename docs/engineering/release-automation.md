...
This is the single stack-wide process, documented once in `wicket-atlas/conventions/release-automation.md`. This file is a stub — see that doc for the canonical process; do not restate or diverge from it here.
...
Default is a **patch** bump (`1.0.0` to `1.0.1`). Override with a marker anywhere in the merge commit message. With squash-merge the PR title becomes that message, so the marker can go in the PR title.

| Marker | Result | Example |
|---|---|---|
| _(none)_ | patch | `1.0.0` to `1.0.1` |
| `#minor` | minor | `1.0.0` to `1.1.0` |
| `#major` | major | `1.0.0` to `2.0.0` |
...
```bash
php .ci/changelog.php 1.0.1 1.0.0..1.0.1
```
